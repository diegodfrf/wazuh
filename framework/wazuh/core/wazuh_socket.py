...
class WazuhAsyncSocket:
    ...
    async def is_connection_lost(self):
        """Check if connection is lost."""
        return self.writer.is_closing() or await self.writer.wait_closed()
    ...
    async def send(self, msg_bytes: bytes, header_format: str = "<I"):
        ...
        WazuhException(1014)
            If the message length was 0 or there are problems with the socket.
...
class WazuhAsyncSocketJSON(WazuhAsyncSocket):
    ...
    async def send(self, msg_bytes: str, header_format: str ="<I") -> bytes:
        """Converts the message from JSON format to bytes and send it to the socket. 
    ...
    async def receive(self, header_format="<I", header_size=4, raw = False) -> dict:
